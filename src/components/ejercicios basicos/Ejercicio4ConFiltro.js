...
function Ejercicio4ConFiltro() {
  ...
  const [peliculasFiltrada, setPeliculaFiltrada] = useState([]);
  ...
  const filtrarGenero = (genero) => {
    ...
    peliculas.map((pelicula) => {
      if (Array.isArray(pelicula.categoria)) {
        pelicula.categoria.map((categoria) => {
          if (categoria == genero) {
            peliculasFiltradas.push(pelicula);
          }
        });
      } else {
        if (pelicula.categoria == genero) {
          peliculasFiltradas.push(pelicula);
        }
      }
    });
    setPeliculaSeleccionada(peliculasFiltradas[0]);
    setPeliculaFiltrada(peliculasFiltradas);
  };
  ...
  return (
    <div>
      <Navbar bg="warning" variant="">
        <Container>
          <Navbar.Brand href="/#" onClick={() => setPeliculaFiltrada([])}>
            <img src="/Palace-Logo.png" width="100" height="50" className="d-inline-block align-top" alt="Logo" />
          </Navbar.Brand>{" "}
          <Nav className="me-auto">
            <NavDropdown title="Categorías " id="basic-nav-dropdown">
              {generos.map((genero) => (
                <NavDropdown.Item onClick={() => filtrarGenero(genero)}>{genero}</NavDropdown.Item>
              ))}
            </NavDropdown>
      ...
      <Container>
        ...
        <Row>
          {peliculasFiltrada.length === 0
                    ...
                    <CardBody>
                      ...
                      peliculaSinopsis.sinopsis === pelicula.sinopsis ? (
                        <>
                          ...
                          <CardText className="card-body "> {peliculaSinopsis.sinopsis}</CardText>
                        </>
                      ) : null}
                    </CardBody>
                    <div>
            ...
            : peliculasFiltrada.map((pelicula) => (
                <Col md={4} key={pelicula.id}>
                  <Card>
                    <CardImg src={pelicula.foto} alt={pelicula.nombre} style={{ maxHeight: "600px" }} />
                    <CardBody>
...
export default Ejercicio4ConFiltro;
